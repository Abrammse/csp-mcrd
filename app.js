...
(function () {
  ...
  $('#btn-search-sijil')?.addEventListener('click', function() {
    ...
    setTimeout(() => {
      ...
      hide($('#check-result'));
      hide($('#btn-goto-settlement-from-company'));
  ...
  $('#btn-check-company')?.addEventListener('click', function () {
    ...
    setTimeout(() => {
      ...
      if (currentSijil === '123' || currentSijil === '١٢٣') {
        ...
        show($('#check-result'));
        show($('#btn-goto-settlement-from-company'));
      }
    }, 1000);
  });

  $('#btn-goto-settlement-from-company')?.addEventListener('click', () => navigateTo('screen-settlement'));
  $('#btn-back-company-from-settlement')?.addEventListener('click', () => navigateTo('screen-company'));
  ...
  bindTabs('#comp-tabs-nav', '#comp-tabs-content');
  ...
  // Employee Actions
  bindTabs('#emp-comp-tabs-nav', '#emp-comp-tabs-content');

  $('#emp-btn-approve')?.addEventListener('click', () => {
    hide($('#emp-action-buttons'));
    show($('#emp-action-approve-flow'));
  });

  $('#emp-btn-return')?.addEventListener('click', () => {
    hide($('#emp-action-buttons'));
    show($('#emp-action-return-flow'));
  });

  $('#emp-btn-cancel-return')?.addEventListener('click', () => {
    hide($('#emp-action-return-flow'));
    show($('#emp-action-buttons'));
  });

  $('#emp-paid-check')?.addEventListener('change', function() {
    if (this.checked) {
      show($('#emp-upload-letter-section'));
    } else {
      hide($('#emp-upload-letter-section'));
    }
  });

  $('#emp-btn-send-return')?.addEventListener('click', function() {
    this.disabled = true;
    this.innerHTML = '<span class="spinner"></span> جاري الإرسال...';
    setTimeout(() => {
      this.parentNode.innerHTML = '<div class="alert alert-success" style="margin:0; text-align: right; direction: rtl;"><div class="alert-content"><div class="alert-title">تم إرسال الاستيفاء للمستثمر بنجاح ✓</div></div></div>';
    }, 1000);
  });

  $('#emp-final-complete-check')?.addEventListener('change', function() {
    if (this.checked) {
      const resultDiv = $('#emp-final-result');
      resultDiv.innerHTML = `
        <div class="alert alert-info" style="border-color: var(--primary);">
          <div class="alert-content">
            <div class="alert-title" style="display: flex; align-items: center; gap: 8px; color: var(--primary);">
              ...
              جاري تحديث البيانات في مصر المقاصة...
            </div>
          </div>
        </div>
      `;
      ...
      setTimeout(() => {
        resultDiv.innerHTML = `
          <div class="alert alert-success">
            <div class="alert-content">
              <div class="alert-title">تم التحديث بنجاح، ملف الشركة مكتمل ✓</div>
            </div>
          </div>
        `;
      }, 1500);
    } else {
      hide($('#emp-final-result'));
    }
  });
